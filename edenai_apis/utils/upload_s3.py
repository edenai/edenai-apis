...
def get_amazon_s3_client():
    """
    Get and instanciate amazon s3 client
    """
...
def upload_file_to_s3(file: BufferedReader, file_name: str):
    ...
    get_amazon_s3_client().upload_fileobj(file, BUCKET, filename)
    return get_s3_file_url(filename)


def get_s3_file_url(filename: str):
    """Get url of a file hosted on s3"""
    response = get_amazon_s3_client().generate_presigned_url(
        "get_object",
        Params={
            "Bucket": BUCKET,
            "Key": filename,
...
def get_providers_json_from_s3():
    obj = get_amazon_s3_client().get_object(

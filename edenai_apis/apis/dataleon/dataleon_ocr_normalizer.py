...
def extract_entities_for_page(entities: List[Dict[str, Any]], page_index: int) -> Dict[str, Any]:
    """
...
def extract_tables_for_page(tables: List[Dict[str, Any]], page_index: int) -> List[Dict[str, Any]]:
    """
...
def dataleon_financial_parser(original_response: dict) -> FinancialParserDataClass:
    ...
    for document_page in formatted_response:
        extracted_data.append(FinancialParserObjectDataClass(
            ...
            document_metadata=FinancialDocumentMetadata(document_page_number=document_page.get("page_index")),

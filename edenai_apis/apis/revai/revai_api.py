...
class RevAIApi(ProviderInterface, AudioInterface):
    ...
    def _create_vocabulary(self, list_vocabs: list):
        ...
        response = requests.post(
            ...
            data={"custom_vocabularies": [{"phrases": list_vocabs}]},
        )
    ...
    def audio__speech_to_text_async__launch_job(
        ...
        if vocabulary:
            ...
            self._launch_transcribe(
                file_name, content_url, language, profanity_filter, vocab_name, True
            )
            return AsyncLaunchJobResponseType(
                provider_job_id=f"{vocab_name}EdenAI{file_name}"
            )
    ...
    def audio__speech_to_text_async__get_job_result(
        ...
        try:  # check if job id of vocabulary or not
            config_content = storage_clients(self.api_settings_aws)[
                "speech"
            ].meta.client.get_object(
                Bucket=self.bucket_name, Key=f"{provider_job_id}_settings.txt"
            )
            config = json.loads(config_content["Body"].read().decode("utf-8"))
            # check if vocab finished
            if job_id := not config.get(
                "provider_job_id"
            ):  # check if transcribe have been launched
        ...
        if status == "transcribed":
            ...
            for monologue in original_response["monologues"]:
                text += "".join([element["value"] for element in monologue["elements"]])

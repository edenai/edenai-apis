        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if original_response["Results"][0]["HireAbilityJSONResults"][0].get(
                    ...
                    errors = original_response["Results"][0]["HireAbilityJSONResults"][
                        0
                    ]["ProcessingErrors"][0]
                    raise ProviderException(
                        errors["Error"][0]["ErrorMessage"],
                        code=errors["Error"][0]["ErrorCode"],
                    )
                ...
                for i in infos.get("EducationOrganizationAttendance") or []:
                    ...
                    if isinstance(i.get("ReferenceLocation"), dict):
                        location = ResumeLocation(
                            country_code=i.get("ReferenceLocation", {}).get(
                                "CountryCode"
                            ),
                            region=i.get("ReferenceLocation", {}).get(
                ...
                for i in infos.get("PositionHistory", []):
                    work_location = ResumeLocation(
                        country_code=(i.get("ReferenceLocation") or {}).get(
                            "CountryCode"
                        ),
                ...
                duration = infos.get("TotalYearsOfExperience")
                work = ResumeWorkExp(
                    total_years_experience=duration, entries=work_entries
                )

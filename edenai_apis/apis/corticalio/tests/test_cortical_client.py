...
class TestCorticalClient:
    ...
    def cortical_client(self):
        return CorticalClient({
            "api_key": "abc",
            "base_url": "http://api.cortical.io/"
        })

    @pytest.fixture
    def keywords_cortical(self):
        api_folder = pathlib.Path(__file__).parent.parent.resolve()
        outputs_path = api_folder / 'outputs' / 'text' / 'keyword_extraction_output.json'
    ...
    def test_keyword_normalization(self, keywords_cortical):
        ...
        for n_item in normalised_items:
            s_item = [i for i in standardized_items if n_item.keyword == i.get("keyword")][0]
    ...
    def test_text_sample(self, cortical_client, keywords_cortical):
        ...
        responses.add(responses.POST, 'http://api.cortical.io/keywords',
                      status=200, json=keywords_response)

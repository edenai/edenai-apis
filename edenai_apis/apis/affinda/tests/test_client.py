...
class TestClient:
    ...
    def test_get_organization__with_good_id(self):
        organizations = self.client.get_organizations()
        edenai_organization = [orga for orga in organizations if orga.name == 'Eden AI'][0]
    ...
    def test_get_organization__with_bad_id(self):
        with pytest.raises(expected_exception=ProviderException, match="404 Client Error: Not Found for url:"):
    ...
    def test_current_organization_property(self):
        ...
        assert self.client.current_organization is None, "current_organization must be None before any assigment"

        # Test current_organization.setter
        self.client.current_organization = edenai_organization.identifier
        assert self.client.current_organization is not None, "current_organization cannot be None after assigment"
        ...
        assert self.client.current_organization is None, "current_organization must be None after deletion"

    def test_get_workspaces(self):
        #Test before assigment the organization
        with pytest.warns(expected_warning=Warning, match="You didn't have set a current_organization. Please see the documentations."):
            workspaces = self.client.get_workspaces()
            assert workspaces == [], "get_workspaces must be return an empty list if current_organization doesn't set"
    ...
    def test_current_workspace_property(self):
        ...
        assert self.client.current_workspace is None, "current_workspace must None before assigment"
        ...
        assert self.client.current_workspace is None, "current_workspace must be None after deletion"

    def test_create_and_delete_workspace(self):
        # Test before set current_organization
        del self.client.current_organization
        with pytest.raises(expected_exception=AttributeError, match="You didn't have set a current_organization. Please see the documentations."):
        ...
        new_workspace = self.client.create_workspace(name="Test-Workspace", visibility="organization")
    ...
    def test_get_collections(self):
        ...
        with pytest.warns(expected_warning=Warning, match="You didn't have set a current_workspace. Please see the documentations."):
            collections = self.client.get_collections()
            assert collections == [], "get_collections must be return an empty list when called before current_workspace's assigment"

        # Setup Workspace
        workspaces = self.client.get_workspaces()
        invoice_workspace = [workspace for workspace in workspaces if workspace.name == 'Accounts Payable'][0]
    ...
    def test_get_collection__with_good_id(self):
        ...
        specific_collection = self.client.get_collection(identifier=collection.identifier)
    ...
    def test_current_collection_property(self):
        ...
        assert self.client.current_collection is None, "current_collection must be None before assigment"


        # Test current_collection.setter
        self.client.current_collection = collection.identifier
        assert self.client.current_collection is not None, "current_collection cannot be None after assigment"
        assert self.client.current_collection == collection

        # Test current_collection.deletter
        del self.client.current_collection
        assert self.client.current_collection is None, "current_collection muts be None after deletion"
    ...
    def test_create_and_delete_collection(self):
        ...
        del self.client.current_workspace
        with pytest.raises(expected_exception=AttributeError, match="You didn't have set a current_workspace. Please see the documentations."):
            self.client.create_collection(name="Test-collection", extractor=ExtractorType.RESUME)
        ...
        new_collection = self.client.create_collection(name='Test-collection', extractor=ExtractorType.RESUME)

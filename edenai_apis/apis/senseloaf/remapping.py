...
class ResumeMapper:
    ...
    def __map_name(self) -> ResumePersonalName:
        ...
        raw_name = (name.get('title') + ' ' + name.get('firstName') + ' ' + name.get('middleName') + ' ' + name.get('lastName')).strip(' ').replace('  ', ' ')
    ...
    def __map_location(self) -> ResumeLocation:
        location = self.__original_response['contactMethod']['PostalAddress']['Location']
        raw_location = (location['Area'] + ' ' + location['City'] + ' ' + location['State'] + ' ' + location['Country'] + ' ' + location['Zip']).strip(' ').replace('  ', ' ')
    ...
    def __get_summary(self) -> str:
        summaries = [i['sectionText'] for i in self.__original_response['sections'] if i['sectionName'] == 'Summary']
        return ' '.join(summaries)
    
    def __get_objective(self) -> str:
        objectives = [i['sectionText'] for i in self.__original_response['sections'] if i['sectionName'] == 'Objective']
    ...
    def __map_experience(self) -> ResumeWorkExp:
        ...
        self.__standard_response["work_experience"] = ResumeWorkExp(
            total_years_experience=str(self.__original_response['experience']['TotalExperience']),

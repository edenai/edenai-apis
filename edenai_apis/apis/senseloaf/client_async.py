...
class Client:
    ...
    async def __request_async(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            try:
                ...
                if return_type == "headers":
                    self.__last_api_response = dict(response.headers)
                    self.__last_api_response_type = "headers"
                    self.__last_api_response_code = str(response.status_code)
                    return ResponseData(
    ...
    async def __login_async(self, email, password):
        url = f"{self.BASE_URL}/login"
        headers = {
            "Content-Type": "application/json",
        }
    ...
    async def __parse_resume_from_file_async(self, file: str) -> ResponseData:
        url = f"{self.BASE_URL}/api/v2/parse-resume"
    ...
    async def __parse_resume_from_url_async(self, url: str) -> ResponseData:
        parser_url = f"{self.BASE_URL}/api/v2/parse-resume-url"
        data = json.dumps({"resumeUrl": url})
        headers = {"Authorization": f"Bearer {self.__api_key}"}
    ...
    async def __parse_resume_async(
        self, file: Optional[str] = "", url: Optional[str] = ""
    ) -> ResponseData:
        if file == "" and url == "":
            raise ProviderException(
            ...
            return await self.__parse_resume_from_file_async(file)
        elif url != "":
            return await self.__parse_resume_from_url_async(url)
    ...
    async def parse_document_async(
        ...
        """Async version of parse_document for resume parsing."""
        if parse_type.value == "resume":
            return await self.__parse_resume_async(file, url)
        elif parse_type.value == "job_description":
            # JD parsing doesn't have async implementation yet
            raise NotImplementedError(
                "Async job description parsing is not implemented yet."
            )
        elif parse_type.value == "invoice":
            raise NotImplementedError(
                "Invoice parsing is not implemented yet. Reach out to us at team@senseloaf.com for requesting early release"

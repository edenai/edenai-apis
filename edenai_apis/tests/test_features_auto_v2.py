...
from edenai_apis.utils import compare
...
class CommonProvidersSubfeaturesTests:
    ...
    def _test_compute_subfeature_output(self, provider, feature, subfeature, phase=""):
        ...
        args = validate_all_provider_constraints(provider, feature, subfeature, args)

        # TODO renamed variable
        klass = getattr(interface_v2, feature.title())
        ...
        if phase:
            subfeature_methods[0] += f'__{phase}'
        for subfeature_name in subfeature_methods:
            print(subfeature_name)
            func = getattr(klass, subfeature_name)
            func_2 = func(provider=provider)
            if '__launch_job' in subfeature_name:
                result = func_2(**args)
            elif '__get_job' in subfeature_name:
                final_result = func_2(result.provider_job_id)
                while final_result.status == 'pending':
                    final_result = func_2(result.provider_job_id)
                    sleep(1)
                final_result = final_result.dict()
            else:
                final_result = func_2(**args).dict()
        assert final_result
        assert final_result["standardized_response"]

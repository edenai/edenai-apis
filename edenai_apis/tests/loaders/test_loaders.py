...
class TestLoadFeature:
    @pytest.mark.parametrize(
        ('data_feature'),
        [
            FeatureDataEnum.DATA_CLASS,
            FeatureDataEnum.SAMPLES_ARGS
        ]
    )
    def test_valid_load_feature_feature(self, mocker: MockerFixture, data_feature: FeatureDataEnum):
        mocker_loader = mocker.patch(f'edenai_apis.loaders.data_loader.{data_feature.value}')
        load_feature(
...
class TestLoadProvider:
    ...
    def test_valid_load_provider(self, mocker: MockerFixture, data_provider: ProviderDataEnum):
        mocker_loader = mocker.patch(f'edenai_apis.loaders.data_loader.{data_provider.value}')
        load_provider(
